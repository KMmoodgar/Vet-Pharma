                        ...
                        if (!(contact.length() <10))
                        {
                            if (!(password.length()<6)) {
                                if (Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                                    ...
                                    femail.setError("Email is badly formatted");
                                    femail.requestFocus();
                                }
                            }
                            else
                            {
                                fpassword.setError("Minimum 6 letters required");
                                fpassword.requestFocus();
                            }
                        }
                        else {
                            fcontact.setError("Enter 10 digit contact number");
                            fcontact.requestFocus();
                        }
                    }
                    else{
                        fname.setError("Enter only characters");
                        fname.requestFocus();
                    }
                }
                else {
